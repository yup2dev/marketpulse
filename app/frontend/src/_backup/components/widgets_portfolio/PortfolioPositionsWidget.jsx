...
function _buildPositionsColumns(displayCurrency, exchangeRate, formatKRWProp) {
  ...
  return [
    ...
    },
    {
      ...
      align: 'right',
      sortable: true,
      renderFn: (value) => <span className="tabular-nums text-[11px] text-gray-400">{fmtUSD(value)}</span>,
    },
    {
      key: 'openPrice',
      header: '시가',
      ...
      renderFn: (value) =>
        value != null
          ? <span className="tabular-nums text-[11px] text-gray-300">{fmtUSD(value)}</span>
...
export default function PortfolioPositionsWidget({
  ...
  const COLUMNS = _buildPositionsColumns(displayCurrency, exchangeRate, formatKRWProp);
