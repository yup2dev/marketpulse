    ...
    <div className="space-y-6">
      ...
      {activeSection === 'overview' && ownershipData && (
        <>
          ...
          {/* Ownership Pie Chart & Float Info */}
          ...
          {/* Top Institutional Holders Preview */}
      ...
      {activeSection === 'institutional' && (
        <>
          ...
          {institutionalHolders.length > 0 && (
            <div className="bg-[#1a1a1a] rounded-lg p-6 border border-gray-800">
              <h4 className="text-lg font-semibold text-white mb-4">Institutional Ownership Distribution</h4>
              <div className="h-80">
                <ResponsiveContainer width="100%" height="100%">
                  <Treemap
                    data={institutionalHolders.slice(0, 10).map((h, idx) => ({
                      name: h.name?.split(' ')[0] || `Inst ${idx + 1}`,
                      size: h.pct_held || 0,
      ...
      {activeSection === 'insiders' && (
        <>
          {/* Insider Activity Summary Cards */}
          {insiderSummary && (
            <div className="grid grid-cols-4 gap-4">
              <div className="bg-[#1a1a1a] rounded-lg p-5 border border-gray-800">
                <div className="flex items-center gap-2 text-gray-400 text-sm mb-2">
          ...
          {/* Insider Roster/Holdings */}
          {insiderHolders.length > 0 && (
            <div className="bg-[#1a1a1a] rounded-lg p-6 border border-gray-800">
              <h4 className="text-lg font-semibold text-white mb-4 flex items-center gap-2">
                <UserCheck className="text-green-500" size={20} />
                Insider Holdings ({insiderHolders.length})
              </h4>
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-gray-700">
                      ...
                      <th className="text-right py-3 px-4 text-gray-400 font-medium">Shares Held</th>
                      <th className="text-right py-3 px-4 text-gray-400 font-medium">Latest Transaction</th>
                    </tr>
                  </thead>
                  <tbody>
                    {insiderHolders.map((holder, idx) => (
                      <tr key={idx} className="border-b border-gray-800 hover:bg-gray-800/30">
                        <td className="py-3 px-4 text-gray-500">{idx + 1}</td>
                        <td className="py-3 px-4 text-white font-medium">{holder.name}</td>
                        <td className="py-3 px-4 text-gray-400">{holder.position}</td>
                        <td className="text-right py-3 px-4 text-green-400 font-medium">{formatNumber(holder.shares)}</td>
                        <td className="text-right py-3 px-4 text-gray-400">{holder.latest_transaction_date}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
            ...
            {insiderTransactions.length > 0 ? (
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  ...
                  <tbody>
                    {insiderTransactions.slice(0, 20).map((tx, idx) => (
                      <tr key={idx} className="border-b border-gray-800 hover:bg-gray-800/30">
