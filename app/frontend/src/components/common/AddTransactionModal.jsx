/**
 * AddTransactionModal — add/edit a portfolio transaction.
 * - Add mode: auto-fetches OHLC for the picked ticker + date so the user can
 *   one-click apply open/high/low/close as the trade price.
 * - Edit mode: pre-fills from `initialValues`; same price-lookup helper.
 */
...
export default function AddTransactionModal({
  ...
  // Re-init when switching between add/edit or different rows
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    debounceRef.current = setTimeout(async () => {
      ...
      try {
        ...
        setFetchedQuote(null);
        setFetchError(e.detail || 'Price lookup failed');
      } finally {
  ...
  const priceLabels = { open: 'Open', high: 'High', low: 'Low', close: 'Close' };
              ...
              <div className="flex items-center gap-2">
                ...
                <span className="text-[11px] text-gray-400">
                  {isFetchingPrice
                    ? 'Looking up price…'
                    : fetchedQuote
                    ? `${fetchedQuote.date} OHLC — click to apply`
                    : 'Enter ticker + date to auto-fetch price'}
                </span>
                {isFetchingPrice && <Loader2 size={11} className="animate-spin text-cyan-500 flex-shrink-0" />}
              </div>

              {fetchError && (
                <p className="text-[11px] text-yellow-600">{fetchError} — enter price manually</p>
              )}
