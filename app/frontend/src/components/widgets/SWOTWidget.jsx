...
const COLUMNS = [
  {
    ...
    header: 'Category',
    renderFn: (value, row) => (
      <span className={`font-medium text-xs ${row._textClass}`}>{value}</span>
    ),
  },
  {
    key: 'label',
    header: 'Item',
    renderFn: (value) => <span className="text-gray-300 text-xs">{value}</span>,
  },
  {
    key: 'value',
    header: 'Value',
    align: 'right',
    renderFn: (value, row) => (
      <span className={`text-xs font-medium tabular-nums ${row._textClass}`}>{value}</span>
    ),
  },
];
  ...
  const renderTable = () => (
    <div className="h-full flex flex-col">
      <div className="flex-1 min-h-0">
        <CommonTable columns={COLUMNS} data={allRows} searchable={false} exportable compact pageSize={50} />
      </div>
      <div className="flex-shrink-0 border-t border-gray-800 px-4 py-2 text-xs">
        <span className="text-gray-600">AI Analysis</span>
        <span className="ml-2 text-gray-700 text-[10px]">Coming Soon</span>
