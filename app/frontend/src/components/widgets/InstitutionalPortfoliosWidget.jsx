...
const InstitutionalPortfolios = ({ symbol = null }) => {
  ...
  const fetchInstitutions = async () => {
    setLoading(true);
    try {
      const data = await apiClient.get(`${API_BASE}/portfolio/13f/institutions?use_dynamic=false`);
      const list = data.results || [];
      setInstitutions(list);
      // 초기 로드: 첫 3개 기관 자동 선택
      const defaults = list.slice(0, 3);
      setSelectedInstitutions(defaults);
      defaults.forEach(inst => fetchPortfolio(inst.key));
    } catch (error) {
