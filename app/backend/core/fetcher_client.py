...
def _restore(obj: Any) -> Any:
    """모든 중첩 dict를 DotDict로 재귀 변환.
    portfolio.stocks 같은 중첩 리스트 안의 항목도 .symbol 등 속성 접근이 가능해야 한다.
    """
    if isinstance(obj, list):
        return [_restore(item) for item in obj]
    if isinstance(obj, dict):
        return DotDict({k: _restore(v) for k, v in obj.items()})
    return obj
    ...
    async def fetch(
        ...
        **kwargs: Any,
        ...
        payload: Dict[str, Any] = {
            "provider": provider,
            "model": model,
            "params": _json_safe(params),  # date/datetime/Decimal → str/float
            "ttl": 0,  # QueryExecutor가 이미 캐싱 — Fetcher 측 중복 방지
        }
        try:
...
def _json_safe(obj: Any) -> Any:
    """params 딕셔너리 안의 JSON 비직렬화 타입(date/datetime/Decimal 등)을 문자열로 변환.
    httpx json= 는 표준 json.dumps를 사용해 date를 처리 못하므로 사전 변환이 필요하다.
    """
    if isinstance(obj, dict):
