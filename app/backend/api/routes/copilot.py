"""
Copilot — AI 어시스턴트 (OpenBB Copilot 스타일). Claude / ChatGPT / Gemini 멀티 프로바이더.
...
- LLM 키는 'API 키 관리'(Settings → /api/keys, provider=anthropic|openai|gemini)에 저장된
  사용자 키를 우선 사용하고, 없으면 서버 .env(ANTHROPIC_API_KEY/OPENAI_API_KEY/GEMINI_API_KEY) 폴백.
  여럿 등록돼 있으면 anthropic > openai > gemini 순(요청 body의 provider로 강제 선택 가능).
...
- If you must pass explicit start_date/end_date, derive them from `today`.
- Time-series results are usually oldest→newest; "latest" values are at the END of the list. Quote the newest data points unless the user asks for history.
...
- /api/data/{provider}/{model}?param=value — e.g.:
  ...
  - /api/data/fmp/income_statement?symbol=AAPL — NOTE: fmp needs a paid key; free alternatives below
  - /api/data/sec/income_statement?symbol=AAPL&period=annual — free (SEC EDGAR XBRL); also sec/balance_sheet, sec/cash_flow, sec/financials
  - /api/data/yahoo/financials?symbol=AAPL — free; combined income/balance/cash-flow statements
  ...
  - /api/data/nasdaq/economic_calendar?date=YYYY-MM-DD&country=united+states — economic events (actual/consensus/previous, impact)
  - /api/data/nasdaq/earnings_calendar?date=YYYY-MM-DD — earnings schedule (market cap, EPS forecast, report time)
...
### Other
- /api/news?symbol=AAPL&limit=20 — financial news (live provider feed)
- /api/news/events?symbol=AAPL&start_date=&end_date=&limit=100 — crawled news events mapped to stocks
...
Provider fallback: many models are served by MULTIPLE providers (e.g. income_statement: fmp AND sec; financials: fmp, sec, yahoo).
If a provider returns a subscription/authorization error (FMP free keys reject most endpoints), do NOT retry the same provider —
switch to another provider for the same model (error messages include a hint listing them), or consult /api/providers.
...
- chart.annotations (optional): [{x, label}] — vertical event markers on the chart. x must equal one of the
  rows' x_key values. Use for mapping news/events onto a time series.
...
- Example: "TSLA 주가에 뉴스 이벤트 표시해줘" → get_data /api/data/yahoo/stock_price?symbol=TSLA&period=3mo
  AND /api/news/events?symbol=TSLA (fallback: /api/news?symbol=TSLA) → rows=[{"date": "2026-07-01", "close": 251.2}, ...] →
  chart={"type": "line", "x_key": "date", "y_keys": ["close"], "annotations": [{"x": "2026-06-25", "label": "Robotaxi launch"}]}.
  Annotation x dates MUST exist in rows (snap each event to the nearest trading day).

## add_widget — widget_type catalog
Static types: dividend, stock-splits, company-filings, earnings, insider, ownership-institutional, holder-breakdown, management, swot, economic-moat, investment-scorecard, stock-sentiment, financials, news-feed, research-reports, institutional-portfolios, market-ranking, watchlist, screener, alerts, sparkline, comparison, heatmap, correlation, economic-calendar, earnings-calendar, notes, gdp-forecast, inflation-momentum, initial-claims, jobs-breakdown, yield-curve-snapshot, yield-trends, real-rates, fed-balance-sheet, inflation-trends, labor-market-dashboard, pmi, fin-conditions-tab, sentiment-tab, quant-summary, quant-capm, quant-rolling, option-pricing.
                    ...
                    "properties": {
                        ...
                        "annotations": {
                            ...
                            "items": {
                                "type": "object",
                                "properties": {
                                    "x": {"type": "string", "description": "x-axis value — must match a row's x_key value (e.g. a date)"},
                                    "label": {"type": "string", "description": "Short marker label, e.g. news headline"},
                                },
                                "required": ["x"],
                            },
                            "description": "Optional event markers — vertical dashed lines at x positions (e.g. news events overlaid on a price chart)",
                        },
                    },
    ...
    },
    {
        ...
        "parameters": {
            ...
            "properties": {
                ...
                "annotations_json": {
                    "type": "STRING",
                    "description": "Optional: JSON array string of event markers [{\\\"x\\\": \\\"2026-07-01\\\", \\\"label\\\": \\\"headline\\\"}] — vertical lines on the chart",
                },
                "w": {"type": "INTEGER", "description": "Grid width 1-12 (default 6)"},
                "h": {"type": "INTEGER", "description": "Grid height in rows (default 6)"},
            },
...
# ── 도구 정의 — OpenAI Chat Completions function 형식 ─────────────────────────
# Anthropic input_schema는 표준 JSON Schema이므로 그대로 재사용한다.
OPENAI_TOOLS = [
    {
        "type": "function",
        "function": {
...
class ChatRequest(BaseModel):
    ...
    provider: Optional[str] = None     # 'anthropic' | 'openai' | 'gemini' — 미지정 시 자동 선택
...
_DATE_KEYS = ("date", "period_ending", "period", "as_of", "timestamp")


def _list_window(items: list, max_items: int) -> tuple[list, Optional[str]]:
    """리스트 축약 창 선택. 날짜 오름차순 시계열은 뒤쪽(최신)을 남기고,
    그 외(랭킹 등 중요한 것이 앞에 오는 목록)는 앞쪽을 남긴다."""
    ...
    if isinstance(first, dict) and isinstance(last, dict):
        for k in _DATE_KEYS:
            ...
            if fv and lv and str(fv) < str(lv):
                return items[-max_items:], f"... ({len(items) - max_items} older items truncated, latest kept)"
    return items[:max_items], f"... ({len(items) - max_items} more items truncated)"


def _shrink(obj: Any, max_items: int = MAX_LIST_ITEMS) -> Any:
    """리스트를 max_items개로 줄이고 생략 사실을 남긴다(중첩 포함).
    시계열은 최신 쪽을 보존한다 — 앞쪽만 남기면 LLM이 옛날 데이터로 답하게 된다."""
    if isinstance(obj, list):
        window, note = _list_window(obj, max_items)
        out = [_shrink(x, max_items) for x in window]
        if note:
            out.append(note)
        return out
...
def _alt_providers_hint(path: str) -> str:
    """/api/data/{provider}/{model} 실패 시 같은 model을 지원하는 다른 provider 안내.

    FMP처럼 유료 구독이 필요한 provider가 실패해도 LLM이 즉시 대안 경로로
    재시도할 수 있도록 tool 에러 결과에 덧붙인다.
    """
    ...
    try:
        ...
        alts = sorted(
            name for name, p in ProviderRegistry.get_all().items()
            if name != provider and model in p.fetcher_dict
        )
    except Exception:
        return ""
    if not alts:
        return ""
    paths = ", ".join(f"/api/data/{a}/{model}" for a in alts)
    return f" Hint: other providers support '{model}' — retry with {paths}."
...
async def _exec_get_data(request: Request, auth_header: str, tool_input: dict) -> tuple[str, bool]:
    ...
    try:
        ...
        return f"Error calling {path}: {exc}{_alt_providers_hint(path)}", True

    if resp.status_code >= 400:
        detail = resp.text[:500]
        return f"HTTP {resp.status_code} from {path}: {detail}{_alt_providers_hint(path)}", True
...
def _build_dataset_payload(tool_input: dict) -> tuple[Optional[dict], Optional[str]]:
    ...
    # 이벤트 마커 — Anthropic/OpenAI는 chart.annotations, Gemini는 annotations_json 문자열
    annotations = chart.get("annotations")
    if not isinstance(annotations, list) and tool_input.get("annotations_json"):
        try:
            annotations = json.loads(tool_input["annotations_json"])
        except ValueError:
            annotations = None
    clean_annotations = []
    if isinstance(annotations, list):
        for a in annotations[:50]:
            if not isinstance(a, dict) or a.get("x") in (None, ""):
                continue
            marker = {"x": str(a["x"])}
            if a.get("label"):
                marker["label"] = str(a["label"])[:60]
            clean_annotations.append(marker)
    ...
    if chart_type in _CHART_TYPES:
        ...
        if clean_annotations:
            chart_hint["annotations"] = clean_annotations
...
def _context_prefix(context: Optional[dict]) -> str:
    # 오늘 날짜는 항상 주입 — 시스템 프롬프트는 정적(캐시)이라 여기서 전달해야
    # LLM이 학습 시점의 옛날 날짜로 start_date/end_date를 계산하지 않는다.
...
def _resolve_llm(user_id: str, requested: Optional[str]) -> tuple[Optional[str], Optional[str], dict]:
    ...
    for prov in ("anthropic", "openai", "gemini"):   # 여럿이면 이 순서로 우선
...
# ── OpenAI (ChatGPT) 스트리밍 tool 루프 (REST chat/completions, SDK 불필요) ────
...
def _openai_error_message(status_code: int, body: str) -> str:
    try:
        ...
        detail = body[:300]
    if status_code == 401:
        return "OpenAI API 키가 유효하지 않습니다. API 키 관리에서 확인하세요."
    if status_code == 429:
        return "OpenAI API rate limit 또는 크레딧 한도에 걸렸습니다. 잠시 후 다시 시도하세요."
    return f"OpenAI API 오류 ({status_code}): {detail}"
...
async def _stream_openai(
    ...
    oai_messages: list[dict] = [{"role": "system", "content": SYSTEM_PROMPT}] + list(messages)
    ...
    try:
        async with httpx.AsyncClient(timeout=httpx.Timeout(120.0, connect=10.0)) as http:
            for _iteration in range(MAX_TOOL_ITERATIONS):
                ...
                acc_text = ""
                tool_calls: dict[int, dict] = {}   # index → {id, name, arguments(누적)}
                ...
                async with http.stream(
                    ...
                    if resp.status_code >= 400:
                        ...
                        yield _sse("error", {"message": _openai_error_message(resp.status_code, err_body)})
                        return
                    ...
                    async for line in resp.aiter_lines():
                        ...
                        for tc in delta.get("tool_calls") or []:
                            ...
                            if fn.get("name"):
                                slot["name"] = fn["name"]
                                yield _sse("tool_start", {"name": fn["name"]})
                            if fn.get("arguments"):
                ...
                # ── tool 실행 → assistant/tool 메시지로 이어붙여 루프 계속 ────
                calls = [tool_calls[i] for i in sorted(tool_calls)]
                for i, c in enumerate(calls):
                    if not c["id"]:
                        c["id"] = f"call_{_iteration}_{i}"
                oai_messages.append({
                    "role": "assistant",
                ...
                for c in calls:
                    ...
                    if not isinstance(args, dict):
                        args = {}
                    yield _sse("tool", {"name": c["name"], "input": args})
                    result_text, is_error, extra = await _run_tool(
                        c["name"], args, request, auth_header
                    )
                    for ev, data in extra:
                        yield _sse(ev, data)
                    oai_messages.append({
        ...
        log.exception("copilot: OpenAI connection error")
        yield _sse("error", {"message": f"OpenAI API에 연결할 수 없습니다: {exc}"})
...
async def copilot_status(user: User = Depends(get_current_active_user)):
    ...
    return {
        ...
        "model": (
            ...
            else settings.COPILOT_OPENAI_MODEL if provider == "openai"
    ...
    async def event_stream():
        ...
        try:
            if provider == "anthropic":
                ...
                async for chunk in _stream_openai(api_key, messages, request, auth_header):
                    yield chunk
            else:
