...
class FREDNonfarmPayrollFetcher(Fetcher[NonfarmPayrollQueryParams, NonfarmPayrollData]):
    ...
    UNEMPLOYMENT_API = "https://api.stlouisfed.org/fred/series/observations"
    ...
    def extract_data(
        ...
        **kwargs: Any
        ...
        try:
            ...
            response = requests.get(FREDNonfarmPayrollFetcher.FRED_API_URL, params=params, timeout=10)
            ...
            try:
                ...
                if query.start_date:
                    unemployment_params['observation_start'] = query.start_date.isoformat()
                if query.end_date:
                    unemployment_params['observation_end'] = query.end_date.isoformat()

                unemployment_response = requests.get(
                    FREDNonfarmPayrollFetcher.UNEMPLOYMENT_API,
                    params=unemployment_params,
                    timeout=10
                )
                unemployment_response.raise_for_status()
                unemployment_data = {obs['date']: float(obs['value']) for obs in unemployment_response.json().get('observations', [])
