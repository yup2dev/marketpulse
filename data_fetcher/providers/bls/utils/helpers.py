...
async def download_category_series_ids(category) -> tuple[list, dict]:
    ...
    from data_fetcher.utils.provider_errors import EmptyDataError
    from data_fetcher.providers.bls.utils.constants import SURVEY_CATEGORY_MAP, SURVEY_NAMES
...
def open_asset(asset: str) -> Union["DataFrame", dict]:
    ...
    assets_path = Path(str(files("data_fetcher.providers.bls").joinpath("assets")))
...
async def update_static_asset(category: str) -> None:
    ...
    from pathlib import Path
    from data_fetcher.utils.provider_errors import OpenBBError
    from data_fetcher.providers.bls.utils.constants import SURVEY_CATEGORY_NAMES
    from numpy import nan
    from pandas import DataFrame
